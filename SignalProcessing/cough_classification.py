# Taken from https://c4science.ch/diffusion/10770/browse/master/src/DSP.py
# Same place where I got the segmentation algorithim
...
from Code.SignalProcessing.feature_class import features
...
def classify_cough(x, fs, model, scaler):
    ...
    try:
        ...
        data = (fs, x)
        FREQ_CUTS = [(0, 200), (300, 425), (500, 650), (950, 1150), (1400, 1800), (2300, 2400), (2850, 2950),
                     (3800, 3900)]
        features_fct_list = ['EEPD', 'ZCR', 'RMSP', 'DF', 'spectral_features', 'SF_SSTD', 'SSL_SD', 'MFCC', 'CF',
                             'LGTH', 'PSD']
        feature_values_vec = []
...
def get_cough_confidence(amplitudes,frequencys):
    model_path = 'C:\\Users\\habel\\OneDrive\\Desktop\\EPQ Program\\Code\\SignalProcessing\\Models'

    loaded_model = pk.load(open(os.path.join(model_path, 'cough_classifier'),'rb'))
    loaded_scaler = pk.load(open(os.path.join(model_path, 'cough_classification_scaler'), 'rb'))
    return classify_cough(amplitudes,frequencys,loaded_model,loaded_scaler)
